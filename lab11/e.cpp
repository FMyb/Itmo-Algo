...
void add_edge(int u, int v, long long c, int num) {
    edge e1 = {v, c, 0, num};
    edge e2 = {u, 0, 0, num};
...
bool dfs1(int v, vector<edge*> &path, long long mn) {
    if (v == t) {
        ans.push_back(path);
        ans1.push_back(mn);
        for (edge *x : path) {
    ...
    for (auto u : ed[v]) {
        if (e[u].f > 0) {
            path.push_back(&e[u]);
            bool f = dfs1(e[u].v, path, min(mn, e[u].f));
            path.pop_back();
            if (f) return true;
        }
    }
...
int main() {
    ...
    for (int i = 0; i < m; i++) {
        ...
        cin >> u >> v >> c;
        A = max(A, c);
        add_edge(u - 1, v - 1, c, i);
    }
    dinic(A);
    vector<edge*> path;
    while (dfs1(0, path, INF)) {}
    cout << ans.size() << '\n';
